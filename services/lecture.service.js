...
export default {
  getInfoLecture: async (lectureId) => {
    try {
      const sql = `
        SELECT lecture_id, section_id, lecture_order, lecture_title, description, source, length, is_preview
        FROM lectures
        WHERE lecture_id = $1
      `;
  ...
  insert: async (
    ...
    try {
      const sql = `
        INSERT INTO lectures (lecture_id, section_id, lection_order, lecture_title, description, source, length, is_preview)
  ...
  update: async (
    ...
    try {
      const sql = `
        UPDATE lectures
        SET lecture_order=$2, lecture_title=$3, description=$4, source=$5, length=$6, is_preview = $7
        WHERE lecture_id=$1`;
